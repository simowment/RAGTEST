...
# No need for config import - using environment variables
...
def main():
    ...
    # 2. Load all Python files from data directory (including VectorBT codebase)
    data_dir = os.path.dirname(DOCS_PATH)  # This should be "data"
    if os.path.exists(data_dir):
        print(f"Loading Python files from data directory '{data_dir}'...")
        try:
            ...
            print(f"Loaded {len(code_docs)} Python files from data directory.")
        except Exception as e:
            print(f"Warning: Could not load Python files from '{data_dir}': {e}")

    # 3. Also check for VECTORBT_CODEBASE_PATH if explicitly set
    if CODE_PATH and os.path.exists(CODE_PATH):
        vectorbt_path = os.path.join(CODE_PATH, "vectorbt")
        if os.path.exists(vectorbt_path):
            print(f"Loading additional codebase from '{vectorbt_path}'...")
            try:
                code_reader = SimpleDirectoryReader(
